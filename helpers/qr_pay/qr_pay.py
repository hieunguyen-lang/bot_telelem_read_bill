...
class QRPay:
    ...
    def generate_qr_pay_pil(self, styles: Optional[dict] = {}) -> Image.Image:
        """
        Trả về QR code dạng PIL.Image (không lưu file), với scale lớn hơn.
        """
        qr = segno.make_qr(self.code)

        # scale mặc định nếu chưa có
        segno_style = styles.copy() if styles else {}
        if 'scale' not in segno_style:
            segno_style['scale'] = 8  # bạn có thể thử 6, 8, 10

        buf = BytesIO()
        qr.save(buf, kind='png', **segno_style)
        buf.seek(0)
